/**
 * Renders SCENARIOS (see scenarios.js) into the dashboard shell in index.html.
 *
 ...
 * `renderScenario(await postToEcoRouter(promptText))` — see README.md.
 */
...
function renderScenarioTabs() {
  ...
  SCENARIOS.forEach((scenario, index) => {
    ...
    btn.addEventListener("click", () => {
      activeIndex = index;
      history.replaceState(null, "", `?scenario=${index}`);
      renderScenarioTabs();
...
function setupThemeToggle() {
  ...
  const stored = localStorage.getItem("ecorouter-theme");
  if (stored) {
  ...
  button.addEventListener("click", () => {
    ...
    localStorage.setItem("ecorouter-theme", next);
  });
}
